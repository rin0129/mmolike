...
@bot.event
async def on_ready():
    banmember = conn.execute("SELECT user_id FROM ban_member ORDER BY user_id").fetchall()
    for ban in banmember:
    ...
    print(cpu)
    count = conn.execute("select count(*) from player").fetchone()
    print(count[0])
    print('------')
...
async def ban(ctx, id=""):
    if ctx.message.author.id == 421971957081309194:
        ...
        if kaitou.content == "ok":
            conn.execute(f"INSERT INTO ban_member values({id})")
...
async def ban(ctx, id=""):
    if ctx.message.author.id == 421971957081309194:
        ...
        kaitou = await bot.wait_for('message', check=lambda messages: messages.author.id == ctx.message.author.id)
        if kaitou.content == "ok":
            conn.execute(f"DELETE FROM ban_member WHERE user_id={id}")
...
async def lvset(ctx, clevel=''):
    if ctx.message.author.id == 421971957081309194:
        ...
        conn.execute("UPDATE channel_status SET boss_level=? WHERE channel_id=?", (boss_level, ctx.message.channel.id,))
        conn.execute("UPDATE channel_status SET boss_hp=? WHERE channel_id=?",
        ...
        if boss_level % MONSTER_NUM in [
            ...
            conn.execute("UPDATE channel_status SET monster=? WHERE channel_id=?", (number, channel_id))
            if boss_level % MONSTER_NUM == 0:
                conn.execute("UPDATE channel_status SET monster=? WHERE channel_id=?", (0, channel_id))
            elif boss_level % 5 == 0:
            ...
            try:
                ...
                if mmsg_react.content == "1":
                  ...
                  # データ10個ごとにページ分け
                    ...
                    playerlist = conn.execute("SELECT user_id, count FROM monster_count ORDER BY count DESC").fetchall()
                    isbot = conn.execute("SELECT user_id FROM player WHERE bot=1 ORDER BY user_id").fetchall()
                    all = []
                    kekka = {}
                    rongaina = []
                    rongai = conn.execute("SELECT user_id FROM item WHERE item_id=-10 ORDER BY item_id").fetchall()
...
async def login_loop():
    ...
    if now == '00:00:00':
        kazu = conn.execute("SELECT user_id FROM login ORDER BY user_id").fetchall()
        for log in kazu:
...
async def login(ctx):
    if ctx.message.author.id in login_zumi:
        ...
        conn.execute(f"INSERT INTO login values({ctx.message.author.id})")
        acount = conn.execute(f"SELECT count FROM item WHERE user_id={ctx.message.author.id} AND item_id=-9").fetchone()
        if not acount:
            conn.execute(f"INSERT INTO item values({ctx.message.author.id}, -9, 1)")
            ...
            bcount = conn.execute(f"SELECT count FROM item WHERE user_id={ctx.message.author.id} AND item_id=-9").fetchone()
            conn.execute(f"UPDATE item SET count={bcount[0] + 1} WHERE user_id={ctx.message.author.id} AND item_id=-9")
            embed = Embed(description=f"""{ctx.message.author.mention}さん、ログインを確認しました。\n現在{bcount[0] + 1}日目です！""")
            await ctx.send(embed=embed)
            embedl = discord.Embed(
                description=f"""名前：**{ctx.message.author.name}**\nid:{ctx.message.author.id}\n日数：{bcount[0] + 1}日目""")
...
async def _attack(ctx, user_id, channel_id):
    ...
    if channel_id in special_monster:
        monster_num = conn.execute("SELECT monster FROM channel_status WHERE channel_id=?", (channel_id,)).fetchone()
...
def win_process(ctx, channel_id, boss_level, monster_name):
    ...
    count_battle_members = conn.execute("SELECT user_id FROM in_battle WHERE channel_id=?", (channel_id,)).fetchall()
...
async def status(ctx, id=""):
    ...
    if id == "":
        ...
        in_battle = conn.execute("SELECT channel_id, player_hp FROM in_battle WHERE user_id=?", (user_id,)).fetchone()
        item_comment = ""
        kazu = conn.execute("select count(*) from item where item_id=-10").fetchone()[0]
        ...
        if in_battle:
            ...
            kazu = conn.execute("select count(*) from item where item_id=-10").fetchone()[0]
            if not battle_channel:  # if deleted the battle_channel
                conn.execute("DELETE FROM in_battle WHERE channel_id=?", (in_battle[0]), )
            else:
                in_battle = conn.execute("SELECT channel_id FROM in_battle WHERE user_id=?", (user_id,)).fetchone()
                in_battle_channel_id = in_battle[0]
    ...
    if ctx.message.author.id == 421971957081309194:
        if id:
            ...
            user_id = int(id)
            player_exp = get_player_exp(ctx, user_id)
            in_battle = conn.execute("SELECT channel_id, player_hp FROM in_battle WHERE user_id=?",
                                     (user_id,)).fetchone()
            item_comment = ""
            kazu = conn.execute("select count(*) from item where item_id=-10").fetchone()[0]
            for my_item in conn.execute("SELECT item_id FROM item WHERE user_id=?", (user_id,)).fetchall():
...
def rank(user_id):
    rongailist = conn.execute("SELECT user_id FROM item WHERE item_id=-10 ORDER BY item_id").fetchall()
    for a in rongailist:
        if user_id == a[0]:
...
@bot.command(name='check', aliases=['c'], pass_context=True, description='自分のステータスを確認する')
async def che(ctx, id=""):
    """自分のステータスを確認する"""
    if not ctx.message.author.id == 421971957081309194:
        return
    # author_name = bot.get_user(ctx.message.author.id)
    ...
    for my_item in conn.execute("SELECT item_id FROM item WHERE user_id=?", (user_id,)).fetchall():
        ...
        if my_item[0] == -9:
            item_comment += "【サポーターの証】を持っている。\n"
    player_level = int(math.sqrt(player_exp))
    monster_countt = conn.execute(f"SELECT count FROM monster_count WHERE user_id={user_id}").fetchall()[0]
    my_items = conn.execute("SELECT item_id, count FROM item WHERE user_id=? ORDER BY item_id",
                            (user_id,)).fetchall()
    item_list = "\n".join("{} : {}個".format(items[i[0]], i[1]) for i in my_items)
    status_comment = f"<@{user_id}>のステータス\nLv: {player_level}\nHP: {player_level * 5 + 50} \n攻撃力: {player_level * 2 + 10}\nEXP: {player_exp}\n次のレベルまで {(player_level + 1) ** 2 - player_exp}exp\n{item_comment}\nプレイヤーランクは{rank}位だ！"
    ...
    embed.add_field(
        name="敵を倒した数",
        value=f"{monster_hyouzi(user_id)}体"
    )
    embed.add_field(
        name="プレーヤーランク",
        value=f"{rank}位"
...
async def exp(ctx, mentions, addexp=''):
    ...
    if ctx.message.author.id == 476567173775753227 or ctx.message.author.id == 421971957081309194:
        ...
        conn.execute("UPDATE player SET experience=? WHERE user_id=?", (afterexp, user_id,))
        next_exp = afterexp
...
def get_boss_level_and_hp(channel_id):
    channel_status = conn.execute("SELECT boss_level, boss_hp FROM channel_status WHERE channel_id=?",
                                  (channel_id,)).fetchone()
    rongaina = conn.execute(f"SELECT user_id FROM item WHERE item_id=-10 ORDER BY user_id")
    rongai = []
    for a in rongaina:
        rongai.append(a[0])
    if not channel_status:
        conn.execute("INSERT INTO channel_status values( ?, ?, ?, ?, ?)", (channel_id, 1, 50, 1, 0))
        channel_status = [1, 50]
...
def monster_count(user_id):
    monstercount = conn.execute(f"SELECT count FROM monster_count WHERE user_id=?", (user_id,)).fetchone()
    if monstercount:
        conn.execute("UPDATE monster_count SET count=? WHERE user_id=?", (monstercount[0] + 1, user_id,))
    else:
        conn.execute("INSERT INTO monster_count VALUES(?,0)", (user_id,))


def monster_hyouzi(user_id):
    monstercount = conn.execute("SELECT count FROM monster_count WHERE user_id=?", (user_id,)).fetchone()
    if monstercount:
        m = monstercount[0]
    else:
        conn.execute("INSERT INTO monster_count VALUES(?,1)", (user_id,)).fetchone()
        m = 1
    return m
...
def fbi_obtain_an_item(user_id, item_id):
    item_count = conn.execute("SELECT count FROM item WHERE user_id=? and item_id=?", (user_id, item_id)).fetchone()
    if item_count:
        conn.execute("UPDATE item SET count=? WHERE user_id=? and item_id=?", (item_count[0] + 100, user_id, item_id,))


def e_obtain_an_item(user_id):
    item_count = conn.execute("SELECT count FROM item WHERE user_id=? and item_id=?", (user_id, 1)).fetchone()
    if item_count:
        conn.execute("UPDATE item SET count=? WHERE user_id=? and item_id=?", (item_count[0] + 10, user_id, 1,))
    else:
        conn.execute("INSERT INTO item VALUES(?,1,1)", (user_id,))
...
async def _atk(message, user_id, channel_id):
    player_hp, error_message = into_battle(user_id, channel_id)
    embed = discord.Embed(description=error_message,
                          color=0xff0000)
    if error_message: return await message.channel.send(embed=embed)
    ...
    if boss_hp <= 0:
        ...
        atk_msg = f"```diff\n{attack_message}```"
        await message.channel.send(atk_msg, embed=embedwin)
        await reset_battle(message, channel_id, level_up=True)
    else:
        conn.execute("UPDATE channel_status SET boss_hp=? WHERE channel_id=?", (boss_hp, channel_id,))
        boss_attack_message = boss_attack_process(user_id, player_hp, player_level, monster_name, boss_level,
                                                  channel_id)
        ...
        embed = discord.Embed(description=f"```diff\n{attack_messagee}```",
                              color=0x36393f)
        await message.channe.send(embed=embed)
        return
...
async def mamenoki_syoukan(ctx, num=""):
    if ctx.author.id == 421971957081309194:
        ...
        if num in numm:
            ...
            channel_id = ctx.message.channel.id
            conn.execute("UPDATE channel_status SET monster=? WHERE channel_id=?", (num, channel_id))
            monster_num = conn.execute("SELECT monster FROM channel_status WHERE channel_id=?",
                                       (channel_id,)).fetchone()
